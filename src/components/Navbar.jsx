...
const Navbar = () => {
  ...
  return (
    <nav className={`${styles.paddingX} py-5 w-full flex items-center top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between max-w-7xl mx-auto'>
        ...
        onClick={()=> {
         ...
         >
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer'>Grace </p>
         </Link>
         <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((link,i )=> (
            ...
            ${active=== link.title ? "text-white": "text-secondary"}
             hover:text-white text-[18px] font-medium cursor-auto`}
             onClick={()=> {
